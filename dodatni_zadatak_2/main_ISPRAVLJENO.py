...
def detekcija_slova_p(image_path):
    ...
    for k in konture:
        ...
        # ### SLEDECA 2 IF-A SU IZMENA JER JE PROFESOR ZAMERIO ZASTO JA FILTRIRAM TE OBJEKTE DA AKO SU
        # ### MANJI OD 100 DA IH NE PROVERAVAM JER IPAK MODEL TREBA SVE DA ODLUCI TAKO DA SAMO JE TO IZMENA, OSTALO JE DOBRO

        # IZMENA: Uklonjen uslov 'povrsina < 100'.
        # sada dozvoljavamo i manjim konturama da prodju do modela jer model sam treba da odluci
        # zadrzavamo samo filter za prevelike objekte (celu sliku)

        # -----------------------------------------------------------------

        if povrsina > maksimalna_povrsina:
            continue

        # dodatna provera: ignorisemo samo ekstremno male mrlje (npr. 2x2 piksela) da izbegnemo greske pri resize-u
        if w < 3 or h < 3:
            continue
        ...
        # IZMENA: Posto smo uklonili filter povrsine, ovde se oslanjamo iskljucivo na model.
        # Ako je predikcija visoka, znaci da je model siguran da je to P, bez obzira na to koliko je malo.
        if predikcija > 0.9:
